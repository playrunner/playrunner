...
| `PUBSUB_EMULATOR_PORT`      | `8084`                              | Host port mapped to fixed container port `8085` for the Docker-backed Pub/Sub emulator          |
...
| Variable                                            | Default                                        | Description                                                                                                                                                                                                                                                                     |
| --------------------------------------------------- | ---------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `PORT`                                              | `3011`                                         | Port the Express API server listens on                                                                                                                                                                                                                                          |
...
| `ORCHESTRATOR_PORT`                                 | `3012`                                         | Host port the Orchestrator Docker container is mapped to                                                                                                                                                                                                                        |
| `ORCHESTRATOR_URL`                                  | `http://localhost:3012`                        | Full URL used by the API to communicate with the Orchestrator                                                                                                                                                                                                                   |
...
| `EDITOR_API_URL_DOCKER`                             | `http://host.docker.internal:3011`             | API server URL **as seen from inside Docker containers**; local Playwright runners still use it to upload compressed output archives                                                                                                                                            |
...
| `PLAYRUNNER_AUTHENTICATION_BROWSER_CHANNEL`         | `chrome`                                       | Playwright browser channel used when testing a stored Authentication Profile and no executable path is configured. If the default channel is unavailable, Playrunner falls back to its Chromium executable.                                                                     |
| `GCP_PUBSUB_WORKFLOW_EVENTS_TOPIC`                  | `playrunner-workflow-events`                   | Pub/Sub topic name used for workflow execution events. Local runs create/use this topic in the emulator; GCP runs use the managed topic created by OAuth provisioning or optional Terraform.                                                                                    |
...
templates are stored in the GCP connection config, not in
`apps/api/.env`.
...
| Variable                   | Default                       | Description                                                                                                           |
| -------------------------- | ----------------------------- | --------------------------------------------------------------------------------------------------------------------- |
| `VITE_API_URL`             | `http://127.0.0.1:3011`       | Target for the Vite dev-server proxy (`/api/*` and `/outputs/*`) when you run the frontend outside `./start-local.sh` |
| `VITE_SETUP_INSTALLER_URL` | `http://127.0.0.1:3103`       | Target for the Vite dev-server proxy (`/setup-api/*`) when you run the setup app outside `./start-local.sh`           |
...
| Variable                            | Default                        | Description                                                                                                                                                                                                                                                           |
| ----------------------------------- | ------------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `PORT`                              | `3012`                         | Port the Orchestrator Express server listens on when you run it directly against the standard local API settings                                                                                                                                                      |
| `EDITOR_API_URL`                    | `http://localhost:3011`        | API server URL used by local runners for output archive uploads                                                                                                                                                                                                       |
| `ORCHESTRATOR_EXECUTOR_TIMEOUT_MS`  | `300000`                       | Positive-integer timeout in milliseconds for one package-owned node executor. The default is five minutes. The current API-managed local container uses the default unless you inject an override into a custom launch.                                               |
...
```json
{
  "data": {
    ...
    "editorApiUrl": "http://host.docker.internal:3011",
    "eventTransport": {
      "type": "gcp_pubsub",
...
The API constructs the `github` runner settings from the encrypted server-side
connection immediately before execution. This payload is sent only to the
relevant runner; it is not the public integration response used by the browser.
Repository and branch dropdowns call the GitHub package API and receive only
non-secret metadata.
