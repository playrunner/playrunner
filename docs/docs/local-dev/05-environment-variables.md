...
| `GCP_ORCHESTRATOR_SERVICE_NAME`       | _(required for GCP)_                           | Cloud Run service name the API creates or reuses for the remote Orchestrator.                                                                                                                                                                                                   |
| `GCP_ORCHESTRATOR_IMAGE_URI_TEMPLATE` | _(required for GCP)_                           | URI template for an already-pushed Orchestrator image that Cloud Run can pull. Supports `{projectId}`. Example: `australia-southeast1-docker.pkg.dev/{projectId}/orchestrator/playrunner-orchestrator:latest`.                                                                  |
...
| `GCP_PLAYWRIGHT_JOB_NAME_TEMPLATE`  | `playrunner-{runtime}`         | Optional Cloud Run job name template; supports `{runtime}`, `{version}`, `{cpu}`, and `{memory}`                                             |
