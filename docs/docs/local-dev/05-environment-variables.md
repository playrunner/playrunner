...
| `GCP_ORCHESTRATOR_SERVICE_NAME`       | `playrunner-orchestrator`                      | Required for GCP-backed execution. Default Cloud Run service name the API creates or reuses for the remote Orchestrator; a saved GCP integration setting can override it.                                                                                                       |
| `GCP_ORCHESTRATOR_MIN_INSTANCE_COUNT` | `1`                                            | Required for GCP-backed execution. Minimum warm Orchestrator Cloud Run service instances to keep available.                                                                                                                                                                     |
| `GCP_ORCHESTRATOR_MAX_INSTANCE_COUNT` | `10`                                           | Required for GCP-backed execution. Maximum Orchestrator Cloud Run service instances. Must be greater than or equal to `GCP_ORCHESTRATOR_MIN_INSTANCE_COUNT`.                                                                                                                    |
| `GCP_ORCHESTRATOR_CPU_IDLE`           | `false`                                        | Required for GCP-backed execution. `false` keeps CPU always allocated so the Orchestrator can continue the background DAG run after `/execute` returns.                                                                                                                         |
| `GCP_ORCHESTRATOR_IMAGE_URI_TEMPLATE` | _(required for GCP)_                           | URI template for an already-pushed Orchestrator image that Cloud Run can pull. Supports `{projectId}`. Example: `us-central1-docker.pkg.dev/{projectId}/orchestrator/playrunner-orchestrator:latest`.                                                                           |
