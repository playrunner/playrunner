...
2. **Orchestrator (Cloud Run Service)**: A lightweight Node.js event-loop dispatcher that traverses your visual workflow graphs. It manages state, evaluates dependencies, schedules Playwright Cloud Run Job preparation in the background, and signals each prepared runner over Pub/Sub when its workflow node is ready to execute. Playrunner configures this service from the saved GCP runner settings, including the minimum instance count and CPU idle policy, because `/execute` responds immediately while the service continues the DAG run in the background.
