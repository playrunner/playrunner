...
- Configuration fields that accept workflow or Environment variables must use the shared `Input` or `Textarea` drag-to-field behavior and preserve the complete template expression, such as `{{env.OPENAI_API_KEY}}`. Do not add package-specific drop zones, chips, or parsers that strip `{{...}}`, `env.`, or other template syntax. Resolve credential templates at the trusted orchestration boundary, keep the stored node configuration as a reference rather than a secret value, and pass the resolved secret to runners only through their protected execution environment.
...
- Keep every connection-dialog setup-guide callout consistent with the GCP and GitHub implementations: render a `BookOpen` icon in the standard inset icon container and an `ExternalLink` icon after the guide label. Open the guide in a new tab with `target="_blank"` and `rel="noopener noreferrer"`. When a package imports these icons from `lucide-react`, declare it as an optional peer dependency and a development dependency using the established GCP/GitHub package pattern.
...
- Choose the icon implementation by brand behavior. Fixed-color or multicolor logos such as Jira, Slack, and Playwright use the package-owned SVG URL as the integration `icon` and render as a normal image. Monochrome logos that must adapt to the active theme, such as GitHub and OpenAI, use a package-owned React SVG component with `fill="currentColor"` as the integration `icon`; reuse that component in settings UI and keep any raw asset URL as a separate export. Do not implement theme-adaptive package icons with CSS `mask-image`.
...
- Treat npm publication as an external release mutation. Never run `npm publish`, change npm dist-tags, deprecate, or unpublish a package unless the user explicitly asks for that exact action after the package contents and version are ready for review.
- Before publishing, confirm the intended package name, semantic version, public access, npm registry, and release tag. Check that the version is not already published and that npm authentication belongs to an account authorized for the `@playrunner` scope.
- Run all package-local tests and the required `format:check`, `lint`, and `typecheck` commands. Run the relevant frontend, API, and Orchestrator consumer checks for every declared package surface, then finish with `git diff --check`.
