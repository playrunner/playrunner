...
- Render every instructional callout or instruction box with a visible, context-appropriate Lucide icon aligned to the left of its copy. Use `Info` for general guidance, `BookOpen` when the callout points to setup or documentation, and the matching semantic icon for warnings, errors, and success. Use the established instruction-callout spacing and theme tokens, mark decorative icons `aria-hidden="true"`, and never render instructional copy alone inside a bordered box.
- Render integration authentication choices as the Slack-style segmented control: `flex gap-2 border-b border-subtle pb-2`, compact rounded buttons, and an action-oriented OAuth/provider option selected by default when OAuth is supported. The selected segment uses `bg-[var(--accent)] text-[var(--accent-foreground)]`; unselected segments use muted text with the standard foreground hover. This makes the active option dark in the light theme and white in the dark theme. Include a labelled group and `aria-pressed` on each option.
