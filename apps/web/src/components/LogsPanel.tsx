...
import { ChevronUp, ChevronDown, XCircle, AlertTriangle, Info, Copy, Check, Pin, PinOff, Bug } from "lucide-react";
...
export const LogsPanel = ({ logs = [] }: { logs?: LogItem[] }) => {
  ...
  const [activeFilter, setActiveFilter] = useState<"Error" | "Warning" | "Info" | "Debug" | null>(null);
  ...
  const infoCount = logs.filter(l => l.type === "Info" || l.type === "Build" || l.type === "Log").length;
  ...
  const filteredLogs = activeFilter
    ? logs.filter(l => {
        ...
        if (activeFilter === "Info") return l.type === "Info" || l.type === "Build" || l.type === "Log";
    ...
    >
      ...
      >
        ...
        <div className="flex items-center gap-3 text-[13px] text-neutral-400">
          <div className="flex items-center gap-3 mr-2 font-sans font-medium">
            ...
            <div className={cn(
              ...
              activeFilter === "Error"
                ...
                : errorCount > 0 ? "hover:text-neutral-200 cursor-pointer" : "opacity-30"
            )}
              onClick={(e) => { e.stopPropagation(); setActiveFilter(activeFilter === "Error" ? null : "Error"); }}
              role="button"
              title={activeFilter === "Error" ? "Show all logs" : "Filter by Error"}
            >
            ...
            <div className={cn(
              ...
              activeFilter === "Warning"
                ? "bg-white/10 text-neutral-200"
                : warningCount > 0 ? "hover:text-neutral-200 cursor-pointer" : "opacity-30"
            )}
              onClick={(e) => { e.stopPropagation(); setActiveFilter(activeFilter === "Warning" ? null : "Warning"); }}
              role="button"
              title={activeFilter === "Warning" ? "Show all logs" : "Filter by Warning"}
            >
            ...
            <div className={cn(
              ...
              activeFilter === "Info"
                ? "bg-white/10 text-neutral-200"
                : infoCount > 0 ? "hover:text-neutral-200 cursor-pointer" : "opacity-30"
            )}
              onClick={(e) => { e.stopPropagation(); setActiveFilter(activeFilter === "Info" ? null : "Info"); }}
              role="button"
              title={activeFilter === "Info" ? "Show all logs" : "Filter by Info"}
            >
            ...
            <div className={cn(
              ...
              activeFilter === "Debug"
                ? "bg-white/10 text-neutral-200"
                : debugCount > 0 ? "hover:text-neutral-200 cursor-pointer" : "opacity-30"
            )}
              onClick={(e) => { e.stopPropagation(); setActiveFilter(activeFilter === "Debug" ? null : "Debug"); }}
              role="button"
              title={activeFilter === "Debug" ? "Show all logs" : "Filter by Debug"}
            >
          ...
          <button
            ...
            className={cn("hover:text-white transition-colors", autoScroll ? "text-blue-400" : "text-neutral-600")}
            title={autoScroll ? "Auto-scroll on — click to unpin" : "Auto-scroll off — click to pin"}
          >
      ...
      >
         {filteredLogs.map((log) => {
            ...
            return (
              <div key={log.id} className="group relative flex items-start gap-4 border-b border-[#2A2A2A] px-4 py-[7px] hover:bg-white/[0.02]">
