...
export const NODE_TYPES: AppNodeType[] = [
  ...INTEGRATIONS.filter(i => !HIDDEN_CANVAS_NODES.includes(i.id)).map(i => ({
    id: i.id,
    label: i.name,
    type: i.nodeType || "action",
    color: i.color,
    ...(typeof i.icon === "string" ? { iconSrc: i.icon } : { fallbackIcon: i.icon }),
  }))
];

const PINNED_IDS = ["playwright", "environment", "schedule", "jira", "slack"];

export function NodeSelectorModal({ isOpen, onClose, onSelect }: NodeSelectorModalProps) {
  ...
  pinnedNodes.sort((a, b) => PINNED_IDS.indexOf(a.id) - PINNED_IDS.indexOf(b.id));
    ...
    >
      <div className="w-10 h-10 rounded-lg bg-background border border-subtle flex items-center justify-center shrink-0 shadow-sm p-1.5">
        {node.iconSrc ? (
          ...
          <div className="text-[12px] font-medium tracking-wide">{node.fallbackText}</div>
        ) : null}
      </div>
      <div>
      ...
      <div className="relative w-full max-w-md bg-surface border border-strong rounded-xl shadow-2xl overflow-hidden flex flex-col max-h-[80vh]">
            ...
            <input
              ...
              onKeyDown={(e) => {
                ...
                if (e.key === 'Enter' && filteredNodes.length > 0) {
                  const firstNode = pinnedNodes.length > 0 ? pinnedNodes[0] : otherNodes[0];
                  if (firstNode) onSelect({ typeId: firstNode.id, label: firstNode.label });
                }
              }}
            />
          </div>
        ...
        <div className="p-2 overflow-y-auto">
          {filteredNodes.length === 0 ? (
            <div className="text-center py-8 text-muted text-sm">No tools found</div>
          ) : (
            <>
              {pinnedNodes.length > 0 && (
                <div>
                  <div className="px-3 pt-2 pb-1">
                    <span className="text-[10px] font-semibold text-muted uppercase tracking-wider">Most commonly used</span>
                  </div>
              ...
              {otherNodes.length > 0 && (
                <div>
                  ...
                  <div className="px-3 pt-2 pb-1">
                    <span className="text-[10px] font-semibold text-muted uppercase tracking-wider">All integrations</span>
                  </div>
