...
export function GithubSettingsModal({ isOpen, onClose }: GithubSettingsModalProps) {
             ...
             if (event.data?.params?.code) {
               try {
                  ...
                  if (event.data.params.installation_id) {
                     integrationData.installationId = event.data.params.installation_id;
                  }
                  
                  await DbAPI.saveIntegration(auth.currentUser.uid, "github", integrationData);
               ...
               if (event.data?.params?.setup_action === 'update') {
                 ...
                 alert("Installation successful, but no OAuth code found. Please ensure you checked 'Request user authorization (OAuth) during installation' in your GitHub App settings.");
                 if (popupRef.current) popupRef.current.postMessage({ type: 'oauth_close' }, window.location.origin);
               }
             }
          }
  ...
  return (
    <Modal
      ...
      icon={<div className="w-5 h-5 bg-[var(--foreground)]" style={{ WebkitMaskImage: "url(/images/integrations/github.svg)", WebkitMaskSize: "contain", WebkitMaskRepeat: "no-repeat", WebkitMaskPosition: "center", maskImage: "url(/images/integrations/github.svg)", maskSize: "contain", maskRepeat: "no-repeat", maskPosition: "center" }} />}
      footer={
        !authSuccess ? (
          <button 
            onClick={handleAuthenticateGithub}
            disabled={!githubAppName || !githubClientId || !githubClientSecret || isAuthenticating}
    ...
    >
      {authSuccess ? (
        <div className="flex flex-col items-center justify-center text-center gap-4 py-8">
          ...
          <div>
            <h3 className="text-xl font-semibold text-[var(--foreground)] mb-2">GitHub Connected Successfully</h3>
            ...
            <div className="bg-[var(--control-bg)] border border-[var(--border)] rounded-lg p-4 text-left max-w-sm mx-auto mb-6">
              <h4 className="text-sm font-medium text-[var(--foreground)] mb-2">Manage Repositories</h4>
              
              <div className="space-y-3 text-xs text-muted">
                <p>
                  You can change which repositories the app has access to at any time.
                </p>
                {githubAppSlug && (
                  <a 
        ...
        <div className="flex flex-col gap-6">
          <ol className="list-decimal pl-4 space-y-3 text-sm text-[var(--foreground)]">
            <li>
              Go to your <a href="https://github.com/settings/apps" target="_blank" rel="noopener noreferrer" className="text-blue-400 hover:text-blue-300 hover:underline">GitHub Developer Settings</a>.
            </li>
            <li>
              Click <strong>New GitHub App</strong>.
            </li>
            <li>
              Under <strong>Basic information</strong>, give your app a <strong>GitHub App Name</strong> (this becomes your app slug) and set a <strong>Homepage URL</strong>.
            </li>
            <li>
              Under <strong>Identifying and authorizing users</strong>, add the following callback URL:
              ...
              <p className="mt-2 text-xs text-blue-400 font-medium">IMPORTANT: You must check "Request user authorization (OAuth) during installation" just below the callback URL.</p>
            </li>
            <li>
              Under <strong>Post installation</strong>, set the <strong>Setup URL</strong> to the exact same URL as above.
              <br />
            ...
            <li>
              Under <strong>Repository permissions</strong>, set <strong>Contents</strong> to <strong>Read and write</strong>.
            </li>
            <li>
              Under <strong>Where can this GitHub App be installed?</strong>, select:
              <ul className="list-disc pl-4 mt-2 space-y-1 text-muted">
                 <li><strong>Only on this account:</strong> If only you will use this app with your repositories.</li>
                 <li><strong>Any account:</strong> If you plan to let other users connect their own GitHub accounts and use this app with their repositories.</li>
              </ul>
            </li>
            <li>
            ...
            <li>
              Copy and paste the App Name, Client ID, and Secret below. You can decide which repositories to install it on in the next step.
            </li>
          </ol>

          <div className="space-y-4 pt-2 border-t border-subtle">
            <div>
              <label className="block text-xs font-medium text-muted mb-1.5">GitHub App Name (URL Slug)</label>
              <Input 
