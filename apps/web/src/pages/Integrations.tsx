...
export default function Integrations() {
    ...
    const unsubscribeAuth = auth.onAuthStateChanged((user) => {
      ...
      unsubscribes.forEach(unsub => {
        if (typeof unsub === 'function') {
          try { unsub(); } catch (e) { console.error("Failed to unsubscribe", e); }
        }
      });
      unsubscribes = [];
      ...
      unsubscribes = INTEGRATIONS.map(integration => {
        ...
        const authPath = integration.getAuthPath ? integration.getAuthPath(user.uid) : "";
        
        if (authPath.includes("cloud_credentials")) {
          if (typeof DbAPI.subscribeToCloudCredential === 'function') {
            return DbAPI.subscribeToCloudCredential(user.uid, integration.id, (exists) => {
              setConnections(prev => ({ ...prev, [integration.id]: exists }));
            });
          } else {
            console.error("DbAPI.subscribeToCloudCredential is not a function. HMR issue?");
            return () => {};
          }
        }
        
        return DbAPI.subscribeToIntegration(user.uid, integration.id, (exists) => {
          setConnections(prev => ({ ...prev, [integration.id]: exists }));
        });
      });
    });
  ...
  const displayIntegrations = INTEGRATIONS.filter(i => i.id !== "playwright" && i.id !== "code" && i.id !== "environment");
    ...
    <>
      ...
      <main className="flex-1 overflow-y-auto max-w-6xl mx-auto p-8">
        <div className="mb-8 flex items-center justify-between">
          <div>
            <h2 className="text-2xl font-bold text-[var(--foreground)] tracking-tight">Connected Integrations</h2>
            <p className="text-muted mt-1">Manage external services and tools connected to your workspace.</p>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {displayIntegrations.map(integration => {
            const isConnected = connections[integration.id] ?? integration.connected;
                ...
                <div className="w-12 h-12 rounded-lg bg-background border border-subtle flex items-center justify-center p-2 shadow-sm">
                  {integration.icon ? (
                    typeof integration.icon === "string" ? (
                      ['github', 'openai', 'webhooks', 'whatsapp'].includes(integration.id) ? (
                        <div 
                    ...
                    <div className="text-lg font-bold text-muted">{integration.name[0]}</div>
                  )}
                </div>
                {isConnected ? (
                  ...
                  <Button variant="secondary" size="sm" onClick={() => { if (integration.SettingsModal) setActiveModal(integration.id); }}>
                    Connect
                  </Button>
                )}
              </div>
              <div className="flex-1">
                <h3 className="text-base font-semibold text-[var(--foreground)]">{integration.name}</h3>
                <p className="text-sm text-muted mt-1 line-clamp-2">{integration.description}</p>
                {isDisabled ? (
                  <p className="text-xs text-muted mt-3">{integration.disabledReason}</p>
                ) : null}
              </div>
              <div className="mt-4 pt-4 border-t border-subtle -mx-5 px-5 flex items-center justify-between self-stretch">
                <span className="text-xs font-medium text-muted uppercase tracking-wider">{integration.category}</span>
