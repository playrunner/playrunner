...
type ConnectionType = 'sequential' | 'concurrent' | 'independent' | 'success' | 'failure';
...
function CloudProviderDropdown({ value, onChange, onOpenSettings, connectedIds, cloudProjectId, providers }: { value: string, onChange: (v: string) => void, onOpenSettings: () => void, connectedIds: Set<string>, cloudProjectId?: string, providers: typeof CLOUD_PROVIDERS }) {
  ...
  return (
    <div className="relative" ref={dropdownRef}>
      <button 
        ...
        title={displayLabel}
      >
        {displayIcon ? <img src={displayIcon} alt={displayLabel} className="w-5 h-5 object-contain" /> : <Monitor className="w-4 h-4 text-muted" />}
        <span className="max-w-[160px] truncate">{displayLabel}</span>
        <ChevronDown className={cn("w-3 h-3 text-muted transition-transform", isOpen && "rotate-180")} />
      </button>
        ...
        <div className="absolute top-full left-0 mt-2 w-64 bg-surface border border-subtle rounded-xl shadow-xl overflow-hidden flex flex-col z-50">
          <div className="p-1">
            ...
            >
              ...
              {isLocal && <span className="w-1.5 h-1.5 rounded-full bg-[var(--accent)] flex-shrink-0" />}
            </button>

            <div className="my-1 border-t border-subtle" />

            {providers.filter((provider) => provider.id !== "LOCAL-DEV").map(provider => {
              ...
              return (
                ...
                >
                  <img src={provider.icon} alt={provider.label} className="w-4 h-4 object-contain flex-shrink-0" />
...
export default function Editor() {
  ...
  const initialConnections = location.state?.initialConnections as Connection[] | undefined;
  ...
  const [connectionStyle, setConnectionStyle] = useState<'step' | 'bezier'>('step');

  const [showNodeSelector, setShowNodeSelector] = useState(false);
  const [nodeSelectorPos, setNodeSelectorPos] = useState<{x: number, y: number} | null>(null);
  const [pendingConnectionSource, setPendingConnectionSource] = useState<DrawingConnection | null>(null);
  ...
  useEffect(() => {
    ...
    const loadWorkflow = async () => {
      ...
      if (auth.currentUser) {
        try {
          const data = await DbAPI.getWorkflow(auth.currentUser.uid, activeWorkflowId);
          if (data) {
            ...
            if (data.cloudProvider) setCloudProvider(normalizeCloudProvider(data.cloudProvider));
            else if (data.provider) setCloudProvider(normalizeCloudProvider(data.provider));
            loaded = true;
          }
        } catch (err) {
      ...
      if (!loaded) {
        ...
        const sampleNodes: NodeData[] = [
          { id: "n1", nodeType: "playwright", label: "Playwright", x: 400, y: 300, width: 128, height: 128 },
          { id: "n2", nodeType: "jira", label: "Jira", x: 800, y: 100, width: 128, height: 128 },
          { id: "n3", nodeType: "google-chat", label: "Google Chat", x: 800, y: 300, width: 128, height: 128 },
          { id: "n4", nodeType: "asana", label: "Asana", x: 800, y: 500, width: 128, height: 128 },
        ];
        
        const sampleConnections: Connection[] = [
          { id: "c1", sourceId: "n1", targetId: "n2", sourcePort: "right", targetPort: "left" },
          { id: "c2", sourceId: "n1", targetId: "n3", sourcePort: "right", targetPort: "left" },
          { id: "c3", sourceId: "n1", targetId: "n4", sourcePort: "right", targetPort: "left" },
        ];
  ...
  const [isDrawingConnection, setIsDrawingConnection] = useState<DrawingConnection | null>(null); // source nodeId
  ...
  const [nodeStatus, setNodeStatus] = useState<Record<string, 'idle' | 'running' | 'success' | 'error' | 'warning'>>({});
  ...
  const [connectedCloudIds, setConnectedCloudIds] = useState<Set<string>>(new Set());
  ...
  const handleCopyCode = async () => {
    try {
      await navigator.clipboard.writeText(JSON.stringify({ nodes: getNodesWithParents(), connections, cloudProvider, concurrency }, null, 2));
      setIsCopied(true);
    ...
    const checkConnections = async (user: any) => {
      ...
      const results = await Promise.all(
        availableCloudProviders
          ...
          .map(async (p) => {
          try {
            const data = await DbAPI.getCloudCredential(user.uid, p.credentialId!);
  ...
  const handlePlay = useCallback(async () => {
    ...
    if (auth.currentUser) {
      ...
      if (currentCloudProvider !== "LOCAL-DEV") {
        try {
          const cloudCreds = await DbAPI.getCloudCredential(auth.currentUser.uid, currentCloudProvider.toLowerCase());
          ...
          console.error(`Failed to fetch ${currentCloudProvider} cloud credentials:`, err);
        }
      }
    } else {
    ...
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      if (e.key === "Backspace" || e.key === "Delete") {
        ...
        setConnections((prev) => prev.filter(
          (c) => !selectedConnectionIds.has(c.id) && 
  ...
  const handlePointerDown = (e: React.PointerEvent) => {
    ...
    if ((e.target as Element).closest('.guard-node') || (e.target as Element).closest('.guard-port')) {
      return;
    }
  ...
  const handlePortPointerDown = (e: React.PointerEvent, nodeId: string, portPos: PortPosition) => {
    e.stopPropagation();
    ...
    if (incomingConnectionIndex !== -1) {
      ...
      setIsDrawingConnection({ fixedNodeId: conn.sourceId, fixedPort: conn.sourcePort || 'right', isForward: true });
      return;
    }

    // Start a new connection from this port
    setIsDrawingConnection({ fixedNodeId: nodeId, fixedPort: portPos, isForward: true });
  };

  const handlePortPointerUp = (e: React.PointerEvent, targetNodeId: string, portPos: PortPosition) => {
    e.stopPropagation();
    if (isDrawingConnection && isDrawingConnection.fixedNodeId !== targetNodeId) {
      const newConnection: Connection = {
        id: `c_${Date.now()}`,
        sourceId: isDrawingConnection.isForward ? isDrawingConnection.fixedNodeId : targetNodeId,
        sourcePort: isDrawingConnection.isForward ? isDrawingConnection.fixedPort : portPos,
        targetId: isDrawingConnection.isForward ? targetNodeId : isDrawingConnection.fixedNodeId,
        targetPort: isDrawingConnection.isForward ? portPos : isDrawingConnection.fixedPort
      };
  ...
  const handleAddNodeFromSelector = (data: { typeId: string, label: string }) => {
    const pos = nodeSelectorPos || screenToCanvas(window.innerWidth / 2, window.innerHeight / 2);
    ...
    if (pendingConnectionSource) {
      const newConnection: Connection = {
        id: `c_${Date.now()}`,
        sourceId: pendingConnectionSource.isForward ? pendingConnectionSource.fixedNodeId : newNode.id,
        sourcePort: pendingConnectionSource.isForward ? pendingConnectionSource.fixedPort : 'right',
        targetId: pendingConnectionSource.isForward ? newNode.id : pendingConnectionSource.fixedNodeId,
        targetPort: pendingConnectionSource.isForward ? 'left' : pendingConnectionSource.fixedPort
      };
      setConnections(prev => [...prev, newConnection]);
    }
  ...
  const handleAutoArrange = () => {
    ...
    let currentLayer = nodes.filter(n => (parentCount.get(n.id) || 0) === 0 && connections.some(c => c.sourceId === n.id || c.targetId === n.id)).map(n => n.id);
    ...
    layers.forEach((layer, colIdx) => {
      ...
      const startY = START_Y + (layers.reduce((max, l) => Math.max(max, l.length), 0) * (128 + NODE_GAP_Y) - NODE_GAP_Y) / 2 - totalHeight / 2;
    ...
    if (islandNodes.length > 0) {
      const graphBottom = layers.length > 0
        ? Math.max(...Array.from(newPositions.values()).map(p => p.y)) + 128 + NODE_GAP_Y * 2
        : START_Y;
  ...
  const getPortCoordinates = (node: NodeData, port: PortPosition, offset: number = 0) => {
    switch(port) {
      ...
      case 'right': return { x: node.x + node.width + offset, y: node.y + node.height / 2 };
      case 'bottom': return { x: node.x + node.width / 2, y: node.y + node.height + offset };
      case 'left': return { x: node.x - offset, y: node.y + node.height / 2 };
    }
  };
  ...
  const renderCurve = (
    ...
    if (style === 'step') {
      ...
      const isHorizontal = (port: PortPosition) => port === 'left' || port === 'right';
      const isVertical = (port: PortPosition) => port === 'top' || port === 'bottom';
  ...
  const renderNodeIcon = (nodeTypeId: string) => {
    ...
    if (nodeType) {
      if (nodeType.iconSrc) {
        ...
        return <img src={nodeType.iconSrc} alt={nodeTypeId} className="w-16 h-16 object-contain pointer-events-none" />;
      }
      if (nodeType.fallbackIcon) {
        const Icon = nodeType.fallbackIcon;
        return <Icon className={cn('w-16 h-16 pointer-events-none', nodeType.color)} />;
      }
    }
    return <span className="text-sm font-mono opacity-50 uppercase pointer-events-none">{nodeTypeId.substring(0, 2)}</span>;
  };
      ...
      <>
        ...
        {isEditingName ? (
          <input
            ...
            onBlur={() => { setIsEditingName(false); handleSaveWorkflow(); }}
            onKeyDown={(e) => {
              if (e.key === 'Enter') { setIsEditingName(false); handleSaveWorkflow(); }
  ...
  }, [projectId, workflowName, isEditingName, setHeaderLeft, navigate, handleSaveWorkflow]);
    ...
    <div className="flex-1 relative overflow-hidden select-none">
      ...
      {ActiveCloudSettingsModal ? (
        <ActiveCloudSettingsModal
          ...
          cloudId={activeCloudProvider?.credentialId || cloudProvider.toLowerCase()}
        />
      ) : null}
        ...
        <div className="bg-[#1e1e1e] rounded-lg relative group overflow-hidden">
          ...
          <div className="p-4 max-h-[60vh] overflow-auto select-text cursor-text">
            <pre className="text-sm font-mono text-gray-300">
              {JSON.stringify({ nodes: getNodesWithParents(), connections, cloudProvider }, null, 2)}
            </pre>
          </div>
        </div>
      </Modal>
      ...
      <div className="absolute top-3 right-3 z-20 flex flex-col items-center gap-1">
        ...
        <span className="text-[10px] text-muted font-mono tabular-nums select-none">{Math.round(transform.scale * 100)}%</span>
        <button
        ...
        <button
          onClick={() => setConnectionStyle(prev => prev === 'step' ? 'bezier' : 'step')}
          className="flex items-center justify-center w-8 h-8 rounded-md text-muted hover:text-[var(--foreground)] hover:bg-white/10 transition-colors"
          title={connectionStyle === 'step' ? 'Switch to Bezier Curves' : 'Switch to Step Lines'}
        >
          {connectionStyle === 'step' ? <Route className="w-4 h-4" /> : <Activity className="w-4 h-4" />}
        </button>
        <button
      ...
      >
        ...
        >
          ...
          <svg className="absolute overflow-visible pointer-events-none" style={{ width: 1, height: 1 }}>
            ...
            {connections.map(conn => {
              ...
              const strokeColor = isSelected ? "var(--accent)" : 
                conn.type === 'success' ? "#4ade80" : 
                conn.type === 'failure' ? "#f87171" : 
                "var(--node-border)";

              const strokeDasharray = isTargetRunning ? "6 6" : (conn.type === 'independent' || conn.type === 'concurrent' ? "6 4" : "none");
              ...
              return (
                ...
                >
                  ...
                  <path 
                    d={renderCurve(startCoords.x, startCoords.y, endCoords.x, endCoords.y, sPort, tPort, connectionStyle)} 
                    stroke="transparent" 
                    strokeWidth={20} 
                    fill="none" 
                  ...
                  <path 
                    ...
                    fill="none" 
                    markerEnd={isSelected ? "url(#arrowhead-selected)" : 
                               conn.type === 'success' ? "url(#arrowhead-success)" : 
                               conn.type === 'failure' ? "url(#arrowhead-failure)" :
                               "url(#arrowhead)"}
                    className={cn(
            ...
            {isDrawingConnection && canvasRef.current && (() => {
              const fixedNode = nodes.find(n => n.id === isDrawingConnection.fixedNodeId);
              if (!fixedNode) return null;
              
              const fixedCoordsStart = getPortCoordinates(fixedNode, isDrawingConnection.fixedPort);
              const fixedCoordsEnd = getPortCoordinates(fixedNode, isDrawingConnection.fixedPort, 10);
              const targetPt = screenToCanvas(mousePos.x, mousePos.y);
              
              const startCoords = isDrawingConnection.isForward ? fixedCoordsStart : targetPt;
              const endCoords = isDrawingConnection.isForward ? targetPt : fixedCoordsEnd;
              
              const movingPort = getOppositePort(isDrawingConnection.fixedPort);
              const sPort = isDrawingConnection.isForward ? isDrawingConnection.fixedPort : movingPort;
              const tPort = isDrawingConnection.isForward ? movingPort : isDrawingConnection.fixedPort;
          ...
          {nodes.map(node => {
            ...
            const isNodeConfigured = (n: NodeData) => {
              if (n.nodeType === 'environment') {
                ...
                return vars.some((v: any) => v.key || v.initialValue || v.currentValue);
              }
            ...
            const hasConnection = (port: PortPosition) => 
              connections.some(c => 
                (c.targetId === node.id && (c.targetPort || 'left') === port) ||
                (c.sourceId === node.id && (c.sourcePort || 'right') === port)
              );
              ...
              return (
                ...
                >
                  <div className={cn(
                    ...
                    connected || port === 'right' || port === 'left' ? "opacity-100 w-6 h-6" : "opacity-0 w-6 h-6 group-hover:opacity-100"
                  )} />
                </div>
              );
            };
            ...
            return (
              <div 
                ...
                className={cn(
                  ...
                  isScheduleNode ? "rounded-t-full rounded-b-lg" :
                  node.nodeType === 'environment' ? "rounded-l-[48px] rounded-r-sm" : "rounded-xl",
                  status === 'running' ? "border-transparent" : "border",
                  status !== 'running' && isSelected && "border-[var(--border-strong)] ring-[4px] ring-[var(--border-strong)] shadow-xl",
                  status !== 'running' && !isSelected && "border-[var(--node-border)]"
                )}
                style={{ 
                  left: node.x, 
                  top: node.y, 
              ...
              >
                {status === 'running' && (
                  <div 
                    ...
                    style={
                      isScheduleNode ? { borderRadius: '9999px 9999px 8px 8px' } :
                      node.nodeType === 'environment' ? { borderRadius: '49px 5px 5px 49px' } : undefined
                    }
                  >
                ...
                {!isScheduleNode && !connections.some(c => c.sourceId === node.id && (c.sourcePort === 'right' || !c.sourcePort)) && (
                   <div 
                     className="absolute top-1/2 -translate-y-1/2 left-full flex items-center z-[-1]"
                   >
                     ...
                     <button 
                       ...
                       onClick={(e) => {
                         e.stopPropagation();
                         setPendingConnectionSource({ fixedNodeId: node.id, fixedPort: 'right', isForward: true });
                         setNodeSelectorPos({ x: node.x + node.width + 80, y: node.y });
                         setShowNodeSelector(true);
                       }}
                ...
                {!isScheduleNode && node.nodeType !== 'environment' && renderPort('top', '-top-5 left-1/2 -translate-x-1/2')}
                {!isScheduleNode && renderPort('right', '-right-5 top-1/2 -translate-y-1/2')}
                {!isScheduleNode && node.nodeType !== 'environment' && renderPort('bottom', '-bottom-5 left-1/2 -translate-x-1/2')}
                {!isScheduleNode && node.nodeType !== 'environment' && renderPort('left', '-left-5 top-1/2 -translate-y-1/2')}
                ...
                {!isNodeConfigured(node) && status !== 'running' && status !== 'success' && status !== 'error' && status !== 'warning' && (
                  <button
                  ...
                  >
                    ...
                    <button 
                      ...
                      onClick={() => {
                        ...
                        setConnections(conns => conns.filter(c => c.sourceId !== node.id && c.targetId !== node.id));
                      }}
                    >
                  ...
                  {editingLabelNodeId === node.id ? (
                    <input
                      ...
                      onBlur={(e) => {
                        ...
                        if (val) setNodes(prev => prev.map(n => n.id === node.id ? { ...n, label: val } : n));
                    ...
                    <span
                      ...
                      onDoubleClick={(e) => {
                        e.stopPropagation();
                        const isLinkedEnvironment = node.nodeType === 'environment' && node.config?.environmentId;
          ...
          {connections.map(conn => {
             ...
             return (
               <div key={`overlay-${conn.id}`} className="absolute z-50 pointer-events-none" style={{ left: midPoint.x, top: midPoint.y }}>
                 
                 {/* Type Label */}
                 {conn.type && conn.type !== 'sequential' && !isHovered && !isMenuOpen && (
                   <div className="absolute -translate-x-1/2 -translate-y-1/2 pointer-events-none">
                      <div className="flex items-center justify-center">
                         <span className={cn(
                           ...
                           conn.type === 'success' ? "bg-green-500/20 text-green-400 border border-green-500/30" :
                           conn.type === 'failure' ? "bg-red-500/20 text-red-400 border border-red-500/30" :
                 ...
                 {isMenuOpen && (
                   <div 
                     ...
                     onPointerEnter={() => setHoveredConnectionId(conn.id)}
                     onPointerLeave={() => { setConnectionMenuId(null); setHoveredConnectionId(null); }}
                     onClick={(e) => e.stopPropagation()}
                   >
                     <div className="px-2 py-1.5 border-b border-subtle mb-1 flex items-center justify-between">
                       <h4 className="text-xs font-semibold text-[var(--foreground)]">Line Condition</h4>
                       <button onClick={(e) => { e.stopPropagation(); setConnectionMenuId(null); }} className="text-muted hover:text-[var(--foreground)] p-0.5 rounded hover:bg-surface-hover">
                          <X size={12} />
                       </button>
                     </div>
                     {(['sequential', 'success', 'failure', 'concurrent', 'independent'] as ConnectionType[]).map(type => (
                       <button
                           key={type}
                           onClick={(e) => {
                             e.stopPropagation();
                             setConnections(prev => prev.map(c => c.id === conn.id ? { ...c, type } : c));
                             setConnectionMenuId(null);
                           }}
                           className={cn(
                             "flex items-center justify-between px-2 py-1.5 text-xs rounded hover:bg-surface-hover transition-colors",
                             conn.type === type || (!conn.type && type === 'sequential') ? "text-blue-400 bg-blue-500/10" : "text-muted hover:text-[var(--foreground)]"
                           )}
                       >
                           <span className="capitalize">{type}</span>
                           {(conn.type === type || (!conn.type && type === 'sequential')) && <CheckCircle2 size={14} />}
                       </button>
                     ))}
                   </div>
                 )}
      ...
      {contextMenu && (
         ...
         >
            <div className="px-2 py-1.5 border-b border-subtle mb-1 flex items-center justify-between">
              <h4 className="text-xs font-semibold text-[var(--foreground)]">Node Options</h4>
              <button onClick={(e) => { e.stopPropagation(); setContextMenu(null); }} className="text-muted hover:text-[var(--foreground)] p-0.5 rounded hover:bg-surface-hover">
                 <X size={12} />
              </button>
            </div>
            {(() => {
              ...
              return (
                <>
                  ...
                  {node?.nodeType === 'playwright' && (
                    <button 
                       className={cn("w-full text-left px-2 py-1.5 text-xs rounded transition-colors", hasReport ? "text-muted hover:text-[var(--foreground)] hover:bg-surface-hover" : "text-muted opacity-50 cursor-not-allowed")}
                       onClick={() => {
                          if (hasReport) {
      ...
      {/* Node Settings Modal */}
      {(() => {
        const node = openNodeSettingsId ? nodes.find(n => n.id === openNodeSettingsId) : null;
        let iconElement = undefined;
        if (node) {
          const matchedType = NODE_TYPES.find(n => n.id === node.nodeType);
          if (matchedType) {
            if (matchedType.iconSrc) {
               iconElement = <img src={matchedType.iconSrc} alt={node.label} className="w-5 h-5 object-contain" />;
            } else if (matchedType.fallbackIcon) {
               iconElement = <matchedType.fallbackIcon className={cn("w-5 h-5", matchedType.color)} />;
            }
          }
        }
            ...
            {node && (
              <IntegrationConfigPanel
                ...
                onConnectOAuth={(provider) => {
                    ...
                    } else if (provider === 'github' || (!provider && node.nodeType === 'playwright')) {
      ...
      {saveStatus && (
        <div className={cn(
          ...
          saveStatus.type === 'success' ? "bg-emerald-500 text-white" :
          saveStatus.type === 'error' ? "bg-red-500 text-white" :
