...
const getProviderIcon = (provider?: string) => {
  ...
  if (matchedProvider?.icon) {
    return <img src={matchedProvider.icon} alt={matchedProvider.label} className="w-5 h-5 object-contain" />;
  }
...
export default function ProjectDetail() {
        ...
        <div className="flex items-center gap-3">
           <div className="w-8 h-8 rounded-lg bg-[var(--surface-hover)] border border-[var(--border)] flex items-center justify-center shrink-0">
             <span className="text-[var(--foreground)] font-semibold text-xs">
               {project?.title ? project.title.substring(0, 2).toUpperCase() : 'P'}
             </span>
           </div>
           {isEditingTitle ? (
             <input
  ...
  const handleCreateWorkflow = async () => {
    ...
    try {
      ...
      const defaultNodes = [
        { id: envId, nodeType: "environment", label: "Environment", x: 200, y: 300, width: 128, height: 128 },
        { id: pwId, nodeType: "playwright", label: "Playwright", x: 500, y: 300, width: 128, height: 128 },
      ];
      const defaultConnections = [
        { id: `conn-${Date.now()}`, sourceId: envId, targetId: pwId, sourcePort: "right", targetPort: "left" },
      ];
    ...
    <>
        <main className="flex-1 overflow-y-auto p-8 max-w-5xl mx-auto w-full">
          <div className="flex items-center justify-between mb-8">
            <div>
              <h2 className="text-2xl font-semibold text-[var(--foreground)] mb-2">Project Dashboard</h2>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              >
                <div className="flex items-center justify-between mb-4">
                  <div className="flex items-center gap-3">
                     ...
                     <h3 className="text-lg font-medium text-[var(--foreground)]">{wf.title || 'Untitled Workflow'}</h3>
                  </div>
                  <div className="flex items-center gap-2">
                    <div className="flex items-center justify-center w-8 h-8 bg-[var(--surface-hover)] rounded-md border border-subtle" title={`Provider: ${wf.cloudProvider || 'None'}`}>
                ...
                <div className="flex items-center justify-between border-t border-subtle pt-4 mt-auto">
                   <div className="flex gap-2">
                     ...
                     <Badge variant="outline">{wf.connections?.length || 0} Conns</Badge>
                   </div>
            ...
            {workflows.length === 0 && (
              <div className="col-span-1 md:col-span-2 py-12 text-center border border-dashed border-subtle rounded-xl">
                <p className="text-muted mb-4">No workflows found. Create one to get started.</p>
                <Button variant="primary" className="gap-2" onClick={handleCreateWorkflow} disabled={isCreatingWorkflow}>
                  {isCreatingWorkflow ? <Loader2 className="w-4 h-4 animate-spin" /> : <Plus className="w-4 h-4" />}
                  {isCreatingWorkflow ? "Creating..." : "New Workflow"}
                </Button>
              </div>
