...
class PlaywrightExecutionRegistry {
  ...
  async execute(request: PlaywrightExecutionRequest): Promise<void> {
    ...
    if (!backend) {
      throw new Error(`Unsupported cloud provider for Playwright execution: ${cloudProvider}`);
    }
...
async function loadPremiumContribution(): Promise<void> {
  ...
  if (typeof premiumModule.createPremiumOrchestratorRuntimeContribution !== 'function') {
    return;
  }

  const contribution = await premiumModule.createPremiumOrchestratorRuntimeContribution();
...
function resolvePremiumRuntimeEntry(): string | null {
  const candidates = [
    ...
    path.resolve(__dirname, '../../../../../premium/runners/orchestrator/register.mjs'),
