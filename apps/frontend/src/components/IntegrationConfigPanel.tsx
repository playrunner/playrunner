...
const WORKFLOW_INPUT_VARIABLES: WorkflowInputVariable[] = [
  ...
  { path: 'workflow.run.logs', type: 'object' },
  { path: 'workflow.run.logs.all', type: 'array' },
  { path: 'workflow.run.logs.error', type: 'array' },
  { path: 'workflow.run.logs.warn', type: 'array' },
  { path: 'workflow.run.logs.info', type: 'array' },
  { path: 'workflow.run.logs.build', type: 'array' },
  { path: 'workflow.run.logs.debug', type: 'array' },
  { path: 'workflow.run.diagnostics', type: 'object' },
  { path: 'workflow.run.diagnostics.sharding', type: 'array' },
  { path: 'workflow.history', type: 'object' },
  { path: 'workflow.history.runs', type: 'array' },
  { path: 'workflow.history.logs', type: 'object' },
  { path: 'workflow.history.logs.all', type: 'array' },
  { path: 'workflow.history.logs.error', type: 'array' },
  { path: 'workflow.history.logs.warn', type: 'array' },
  { path: 'workflow.history.logs.info', type: 'array' },
  { path: 'workflow.history.logs.build', type: 'array' },
  { path: 'workflow.history.logs.debug', type: 'array' },
];
...
export const IntegrationConfigPanel: React.FC<IntegrationConfigPanelProps> = ({
  ...
  useEffect(() => {
      ...
      if (currentIntegration?.authProviders) {
        ...
        const checkProviders = async () => {
          for (const provider of currentIntegration.authProviders!) {
            try {
              const integration = await DbAPI.getIntegration(
    ...
    const unsubscribe = DbAPI.subscribeToIntegration(
      auth.currentUser.uid,
      integrationId,
      (exists, data) => {
  ...
  return (
    ...
    >
      ...
      {showInputPanel && (
        <>
          <div
            ...
            className="bg-[var(--background)] border-r border-subtle flex flex-col shrink-0 overflow-hidden"
          >
              ...
              {incomingNodes.length > 0 ? (
                incomingNodes.map((inNode) => {
                  ...
                  const toggleCollapse = () =>
                    setCollapsedInputs((prev) => ({
                      ...prev,
                      [inNode.id]: !prev[inNode.id],
                    }));
                  const nodeTypeKey = (
                    inNode.nodeType || inNode.label
                  ).toLowerCase();
                  ...
                  if (inNode.nodeType === 'environment') {
                    const vars =
                      inNode.config?.variables?.filter(
                        (v: any) => v.enabled && v.key,
                      ) || [];
                    if (vars.length === 0) return null;
                    return (
                      <div
                        key={inNode.id}
                        className="space-y-3 border border-subtle rounded-md p-2"
                      >
                        <button
                          onClick={toggleCollapse}
                          className="flex items-center justify-between w-full text-left group"
                                  ...
                                  <>
                                    ...
                                    >
                                      <div className="flex items-center gap-2 pointer-events-none">
                                        ...
                                        <span className="text-[10px] text-muted border border-subtle rounded px-1.5 py-0.5">
                                          array[{inNode.output.media.length}]
                                        </span>
                                      </div>
                                      <button
                                        onClick={(e) => {
                                            ...
                                            return (
                                              ...
                                              >
                                                <div className="flex items-center justify-between">
                                                  <span className="text-xs font-mono text-purple-400 pointer-events-none">
                                                    media[{idx}]
                                                  </span>
                                                  <div className="flex items-center gap-2">
                                                    <button
                                                    ...
                                                    <span className="text-[10px] text-muted border border-subtle rounded px-1.5 py-0.5 pointer-events-none">
                                                      {isVideo
                                                        ? 'video'
                                                        : 'image'}
                                                    </span>
                                                ...
                                                {isExpanded &&
                                                  (isVideo ? (
                                                    <video
                                                      src={mediaUrl}
                                                      controls
                                                      className="w-full h-auto rounded border border-subtle pointer-events-auto"
                                                      onDragStart={(e) =>
                                                        e.preventDefault()
                                                      }
                                                      onClick={(e) =>
                                                    ...
                                                    <img
                                                      src={mediaUrl}
                                                      alt="artifact"
                                                      className="w-full h-auto rounded border border-subtle pointer-events-none"
                                                    />
                                                  ))}
                                              </div>
                                            );
              ...
              <div className="space-y-4">
                ...
                <div className="space-y-1.5">
                  ...
                  <Input
                    ...
                    onChange={(e) =>
                      onChange(nodeId, { ...config, channel: e.target.value })
                    }
                ...
                <div className="space-y-1.5">
                  ...
                  <p className="text-[10px] text-muted">
                    Use {'{{variable}}'} syntax to inject data from Input.
                  </p>
                </div>
              </div>
            </>
      ...
      <div
        ...
        className="bg-[var(--background)] border-l border-subtle flex flex-col shrink-0 overflow-hidden"
      >
        <div className="h-14 p-4 border-b border-subtle flex items-center justify-between shrink-0">
          <h3 className="text-sm font-semibold text-[var(--foreground)]">
            Output
          </h3>
          <span className="text-[10px] text-muted px-2 py-0.5 rounded-full border border-subtle">
            Preview
          </span>
        </div>
        <div className="p-4 flex-1 overflow-y-auto space-y-6">
          {nodeType === 'environment' ? (
            <div className="space-y-6">
              <div className="space-y-3">
                ...
                <div className="space-y-2">
                  ...
                  config.variables.filter((v: any) => v.enabled && v.key)
                    .length > 0 ? (
                    config.variables
                      ...
                      .map((v: any) => (
                        <div
                          key={v.id}
                          className="flex items-center justify-between p-2 rounded bg-surface border border-subtle hover:border-strong transition-colors cursor-default"
                        >
                          <div className="flex items-center gap-2">
                    ...
                    <p className="text-xs text-muted italic">
                      No variables selected for output.
                    </p>
                  )}
                </div>
              </div>
            ...
            <div className="space-y-3">
              ...
              <div className="space-y-2">
                ...
                <div className="flex flex-col gap-1.5 p-2 rounded bg-surface border border-subtle hover:border-strong transition-colors cursor-default">
                  ...
                  <span className="text-[10px] text-muted">
                    URL to the Playwright HTML report
                  </span>
                </div>
                <div className="flex flex-col gap-1.5 p-2 rounded bg-surface border border-subtle hover:border-strong transition-colors cursor-default">
                  <div className="flex items-center justify-between">
                  ...
                  <span className="text-[10px] text-muted">
                    List of video and screenshot artifact URLs
                  </span>
                </div>
              </div>
            </div>
            ...
            <div className="space-y-6">
              ...
              <div className="space-y-3 pt-4 border-t border-subtle">
                ...
                <p className="text-[10px] text-muted leading-relaxed">
                  This node triggers the workflow on a recurring schedule. It
                  has no connections — simply drop it on the canvas and
                  configure the schedule. When the schedule fires, all workflows
                  containing this node will be executed automatically.
                </p>
              </div>
            </div>
          ) : (
