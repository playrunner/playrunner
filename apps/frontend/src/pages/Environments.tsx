...
export default function Environments() {
  ...
  const handleSave = async (env: SavedEnvironment) => {
    ...
    const existing = environments.find(
      (e) => e.name.toLowerCase() === env.name.toLowerCase() && e.id !== env.id,
    );
    if (existing) {
      alert(
        `An environment with the name "${env.name}" already exists. Please choose a unique name.`,
      );
      return;
    }
  ...
  const handleDelete = async (e: React.MouseEvent, envId: string) => {
    ...
    if (
      confirm(
        'Are you sure you want to delete this environment? This will not affect nodes that already have variables loaded.',
      )
    ) {
      try {
      ...
      <main className="flex-1 max-w-7xl mx-auto p-8 w-full">
        <div className="mb-8 flex items-center justify-between">
          <div>
            ...
            <p className="text-muted mt-1">
              Manage reusable environment variable collections for your
              workflows.
            </p>
          </div>
        ...
        {isLoading ? (
          ...
          <div className="bg-surface border border-subtle rounded-xl p-12 text-center text-muted">
            ...
            <p className="text-sm">
              Create one to share environment variables across your workflow
              nodes.
            </p>
          </div>
        ) : (
              ...
              >
                ...
                <div className="flex items-center gap-2 mt-3">
                  <Badge variant="outline">
                    {env.variables?.length || 0} variable
                    {(env.variables?.length || 0) !== 1 ? 's' : ''}
                  </Badge>
                  {usageCounts[env.id] > 0 && (
                    <Badge variant="outline">
                      Used in {usageCounts[env.id]} workflow
                      {usageCounts[env.id] !== 1 ? 's' : ''}
                    </Badge>
                  )}
                </div>
                <p className="text-xs text-muted mt-3">
                  {env.updatedAt
                    ? `Last edited ${new Date(env.updatedAt).toLocaleDateString()}`
                    : 'No date'}
                </p>
              </div>
            ))}
