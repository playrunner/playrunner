...
// Proxy endpoint to exchange Jira OAuth code for an access token
...
// Proxy endpoint to refresh Jira OAuth token
...
// Proxy endpoint to fetch Jira projects (since Jira API doesn't support CORS)
jiraRouter.get('/projects', async (req, res) => {
  const token = req.headers['x-jira-auth'];
  if (!token) return res.status(401).json({ error: 'Missing x-jira-auth header' });
  ...
  try {
    ...
    // 2. Fetch projects for the first accessible resource
    const cloudId = resources[0].id;
    const projectsRes = await fetch(`https://api.atlassian.com/ex/jira/${cloudId}/rest/api/3/project?expand=issueTypes`, {
      headers: { 
